...
# Other way: Open a CSV file instead an Excel sheet
library(readr)
planets <- read.csv("C:/Users/DUKE/CLASS DATA/DATA ANALYSIS LAB/planets.csv",
                     header = TRUE, sep = ",", 
                     quote = "\"'", dec = ".")
...
students <- read.csv("~/R/data/student_data.csv")
names <- students[,1]
rownames(students) <- students[,1]
students[,1] <- NULL
students
students[,6] <- 2017 - students[,2]
colnames(students) <- c(colnames(students[1:5]),"Age")
...
# Scree Plot
plot(1:6,eigenvalues,type="b",ylab="Eigenvalues",xlab="Components",main="Scree Plot")
