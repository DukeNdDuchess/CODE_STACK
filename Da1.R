...
# Association Rule mining Mining with R
#############################################################
...
lastfm <- read.csv("~/R/data/lastfm.csv")
head(lastfm)
lastfm[1:20,]
...
musicrules <- apriori(playlist,parameter=list(support=.01,
                                              confidence=.5))
inspect(musicrules)
...
subrules <- musicrules[quality(musicrules)$confidence > 0.5]
